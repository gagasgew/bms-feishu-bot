...
通过飞书聊天消息远程控制 STM32F103C8T6 单片机上的 LED 灯亮灭。用户在飞书群中 @机器人发送自然语言指令（如"开灯"、"屋里好黑"），系统用大模型解析意图后通过串口控制硬件。

## 参考项目

工科男孙老师视频：树莓派 + OpenClaw + 摄像头 + 舵机 + 飞书聊天。本项目是其简化版：PC 替代树莓派，STM32F103C8T6 替代舵机控制板，LED 替代舵机，先把核心链路跑通。
